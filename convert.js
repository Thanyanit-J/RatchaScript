...
const ThaiToJS = (thaiCode) => {
    let jsCode = thaiCode;
    if (isGaruda) {
        if (thaiCode.startsWith(garuda)) {
            jsCode = jsCode.split(garuda)[1];
        }
        else {
            console.error("Invalid input: code does not start with Garuda");
            console.error("รูปแบบเอกสารผิด: ตราครุฑหาย");
            return;
        }
    }
...
// Example usage
convertAndSave('example/example-javascript.js', 'output_thai.เจเอส', JSToThai);
